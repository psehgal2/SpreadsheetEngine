...
@pytest.mark.parametrize("n", [50])  # Adjust the values as needed
def test_pascals_triangle_speed(n):
    ...
    assert execution_time < 20  # Adjust the threshold as needed

def test_is_caching():
    wb = Workbook()
    wb.new_sheet("Sheet1")
    wb.set_cell_contents("Sheet1", "A1", "5")
    wb.set_cell_contents("Sheet1", "A2", "=A1+10")
    wb.set_cell_contents("Sheet1", "A3", "=A2+10")
    wb.set_cell_contents("Sheet1", "A4", "=A3+10")
    wb.set_cell_contents("Sheet1", "A5", "=A4+10")

    wb.set_cell_contents("Sheet1", "A1", "10")
    assert wb.get_cell_value("Sheet1", "A5") == 50
